...
The deterministic suites and installed-reference build pass against the current local Erenshor/Lunaris assemblies. Release-default 0.6.4 logging keeps detailed movement/writer and route-phase traces behind the existing `Diagnostics/Verbose` setting while retaining bounded failure, cancellation, arrival, and leader-reacquisition outcomes. Keep experimental direct cross-zone Follow OFF until the exact installed binary passes the multi-zone live matrix in `docs/FOLLOW_PLAYABLE_STATE.md`.
